...
# Path to the HDF5 file generated by the Julia script (nips_9d.jl)
...
with h5py.File(data_path, 'r') as f:
    energies = torch.from_numpy(f["energies"][:]).float() #Load energies (1d array)
    
    # Get list of all sample keys
    ...
    print("Pre-allocating tensors to save memory...") #avoids copying everything at once which uses too much memory
    ...
    for i, key in tqdm(enumerate(valid_samples), total=n_samples, desc='Curating'):
        ...
        # 2. Intensity
        # Read (2500, 150). Twhis is already (Nq, Nw), so we DO NOT transpose.
        ...
        # Vectorize parameters, enforce order of parameters
